...
export class HomePillars {
  ...
  // آیتمی که در لحظه باید نمایش داده بشه
  currentItem = computed(() => this.newsdata[this.currentIndex()]);

  // متصل به chevron-down: رفتن به آیتم بعدی
  goNext(): void {
    this.currentIndex.update((i) => (i + 1) % this.newsdata.length);
  }

  // متصل به chevron-up: رفتن به آیتم قبلی
  goPrevious(): void {
    this.currentIndex.update((i) => (i - 1 + this.newsdata.length) % this.newsdata.length);
  }
}
